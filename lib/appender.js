...
class Appender {
  constructor(options) {
    ...
    if (config.stream) {
      this.type = 'stream';
      this.outputStream = config.stream;
    } else {
      switch (config.type) {
        case 'console':
          this.type = 'console';
        ...
        case 'file':
          ...
          break;
        case 'stream':
          throw new Error('append type "stream" is not available for options, please set a writable stream to options.stream to use it.');
        default:
  ...
  append(level, output) {
    ...
    if (this.levels === 'all' || _.includes(this.levels, level)) {
      switch (this.type) {
        case 'console':
        case 'stream':
        ...
        case 'file':
          ...
          break;
        default:
          throw new Error(`Unknown appender type "${this.type}".`);
          break;
      }
    }
  }
...
Appender.Stdout = new Appender({stream: process.stdout, levels: ['trace', 'debug', 'info', 'warn']});
Appender.Stderr = new Appender({stream: process.stderr, levels: ['error', 'fatal']});
